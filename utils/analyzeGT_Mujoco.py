...
def analyze_foot_velocities(csv_dir="Data/CSVFiles", output_dir=None):
    ...
    for csv_file in csv_files:
        ...
        required_cols = ['time', 'sport_foot_speed_body_0', 'sport_foot_speed_body_1', 'sport_foot_speed_body_2']
        if not all(col in df.columns for col in required_cols):
            print(f"  Skipping - missing required columns")
            continue
        
        # Check if force column exists (used to infer contact)
        has_force = 'sport_foot_force_0' in df.columns
        ...
        for run_idx in range(num_runs):
            ...
            # Extract left foot velocities (already provided in the data)
            vel_x = df_run['sport_foot_speed_body_0'].values
            vel_y = df_run['sport_foot_speed_body_1'].values
            vel_z = df_run['sport_foot_speed_body_2'].values
            time = df_run['time'].values
            
            # Use raw force values as contact indicator (no thresholding)
            contact_state = df_run['sport_foot_force_0'].values if has_force else None
            ...
            # Only plot if foot velocity plotting is enabled
            if PLOT_FOOT_VELOCITIES:
                # Velocities are already available - no need for numerical differentiation
