...
def nig_loss(nig_params, target, regularizer_weight):
    """Elementwise NIG NLL plus the evidential regularizer from evidential-deep-learning."""
    gamma, nu, alpha, beta = nig_params
    two_beta_lambda = 2.0 * beta * (1.0 + nu)
    nll = (
        0.5 * torch.log(torch.pi / nu)
        - alpha * torch.log(two_beta_lambda)
        + (alpha + 0.5) * torch.log(nu * (target - gamma).square() + two_beta_lambda)
        + torch.lgamma(alpha) - torch.lgamma(alpha + 0.5)
    )
    regularizer = (target - gamma).abs() * (2.0 * nu + alpha)
    return nll + regularizer_weight * regularizer
...
def compute_accuracy(dataloader, model, contact_criterion=None, der_regularizer_weight=0.0):
    """
    ...
    Args:
        ...
        der_regularizer_weight: NIG evidence regularizer coefficient
    ...
    with torch.no_grad():
        for sample in tqdm(dataloader):
            ...
            velocity_seq, velocity_output, covariance_seq, covariance_output, contact_output, nig_params = model(
            ...
            if nig_params is not None:
                ...
                velocity_loss_each = nig_loss(nig_params, gt_velocity, der_regularizer_weight)
    ...
    metrics = {
        ...
        'velocity_loss': velocity_loss_sum / num_batches,
        'velocity_mae_components': velocity_mae_components,
        'num_pred_contact': num_pred_contact,
        'num_gt_contact': num_gt_contact,
        'total_variance_calibration_ratio': squared_error_sum / (total_variance_sum + 1e-8),
    }
...
def train(model, train_dataloader, val_dataloader, config):
    ...
    run_dir = os.path.join("logsDER", f"run_{timestamp}")
    ...
    der_regularizer_weight = float(config.get('der_regularizer_weight', 1e-2))
    ...
    loss_description = f'NIG negative log-likelihood + {der_regularizer_weight:g} evidence regularizer'
    ...
    for epoch in range(config['num_epoch']):
        ...
        for i, samples in tqdm(enumerate(train_dataloader, start=0)):
            ...
            if use_dense_supervision:
                ...
                nig_params_permuted = tuple(parameter.permute(0, 3, 1, 2) for parameter in nig_params)
                velocity_loss_elementwise = nig_loss(
                    nig_params_permuted, velocity_label_seq, der_regularizer_weight
                )
                ...
                velocity_loss_elementwise = nig_loss(nig_params, velocity_label, der_regularizer_weight)
        ...
        train_metrics = compute_accuracy(
            ...
            der_regularizer_weight=der_regularizer_weight
        )
...
def main():
    ...
    if model_arch != 'tcn':
        raise ValueError("DER velocity training is currently implemented only for model_architecture: 'tcn'.")
    
    if model_arch == 'tcn':
        from contact_cnn import DERTCN
        base_model = DERTCN(
